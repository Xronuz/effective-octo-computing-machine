...
export default function XonadonDetailPage() {
  ...
  const fetchXonadon = async () => {
    ...
    if (!res.ok) {
      if (res.xato?.includes('topilmadi') || (res as any).status === 404) {
        ...
        setError(res.xato || 'Xonadon maʼlumotlarini yuklashda xatolik yuz berdi');
      }
      setLoading(false);
      return;
    }
  ...
  const handleSave = async () => {
    ...
    if (!res.ok) {
      setError(res.xato || 'Saqlashda xatolik yuz berdi');
      setSaving(false);
      return;
    }
  ...
  const handleDelete = async () => {
    ...
    if (!res.ok) {
      setError(res.xato || 'O‘chirishda xatolik yuz berdi');
      setDeleting(false);
      return;
    }
  ...
  if (notFound) {
    return (
      <div className="text-center py-20">
        <p className="text-lg font-medium text-[#0F2033]">Xonadon topilmadi</p>
        <p className="mt-1 text-sm text-gray-500">
          So‘ralgan xonadon mavjud emas yoki o‘chirilgan bo‘lishi mumkin.
        </p>
        <Link to="/xonadonlar" className="btn-primary mt-6 inline-flex">
          Xonadonlar ro‘yxatiga qaytish
        </Link>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-[#0F2033] sm:text-2xl">
          {xonadon.full_address || `Xonadon #${xonadon.id}`}
        </h1>
        ...
        {editing ? (
          ...
          <div className="space-y-4">
            <h2 className="text-base font-semibold text-[#0F2033]">
              Xonadon maʼlumotlarini tahrirlash
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Qo‘shimcha maʼlumot..."
                />
              </div>
            </div>
          ...
          <>
            <h2 className="text-base font-semibold text-[#0F2033] mb-4">
              Xonadon maʼlumotlari
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-3 text-sm">
              <InfoRow label="To‘liq manzil" value={xonadon.full_address} />
              <InfoRow label="Mahalla (MFY)" value={xonadon.mfy_nomi} />
              <InfoRow label="Ko‘cha" value={xonadon.kocha_nomi} />
              <InfoRow label="Uy raqami" value={xonadon.uy_raqami} />
              <InfoRow label="Egasi" value={xonadon.egasi_fio} />
              <InfoRow label="Tel" value={xonadon.egasi_tel} />
              <InfoRow
                label="Lat/Lng"
                value={
              ...
              />
              <InfoRow label="Izoh" value={xonadon.izoh} />
              <InfoRow
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-xl shadow-xl p-6 w-full max-w-sm mx-4">
            ...
            <p className="text-sm text-gray-600 mb-4">
              Bu amalni qaytarib bo‘lmaydi. O‘chirishni tasdiqlash uchun xonadon raqamini
              kiriting:
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-sm font-medium text-[#0F2033]">
                        {TURI_LABEL[muammo.turi] || muammo.turi}
                      </span>
                      <span className={STATUS_BADGE[muammo.status] || 'badge-gray'}>
                        {STATUS_LABEL[muammo.status] || muammo.status}
                      </span>
                      {muammo.shubhali && (
                        <span className="badge-yellow">Shubhali</span>
                      )}
                    </div>
                    {muammo.tavsif && (
                    ...
                    <div className="flex items-center gap-3 mt-1.5 text-xs text-gray-400">
                      <span>
                        {new Date(muammo.sana).toLocaleDateString('uz-UZ')}
                      </span>
